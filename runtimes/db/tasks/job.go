...
// 	mu      sync.Mutex               // 锁
// 	runners map[string]*task.TaskRun // 任务中具体执行的设备
// 	sec     *task.Task               // 调度器
...
// 	rt := &RuningTask{
// 		ID:      t.ID,
// 		runners: make(map[string]*task.TaskRun),
...
// 	tsk, err := task.NewTask("task", t.ID, t.Title, t.SeNum, false)
// 	if err != nil {
// 		return nil, err
// 	}
// 	rt.sec = tsk
// 	// rt.ctx, rt.cancle = context.WithCancel(mainsignal.MainCtx)
// 	// rt.sec = sch.NewScheduler(t.SeNum)
// 	// if t.SeNum > 0 {
// 	// 	rt.sec = sch.NewScheduler(t.SeNum)
...
// 			ttr, err := rt.sec.AddInterval(
// 				fmt.Sprintf("%d%d", t.ID, v.DeviceID),
// 				fmt.Sprintf("%s%s", clientTitle, proxyTitle),
...
// 					r, err := runner.GetRunner(v.DeviceType, opt)
// 					if err != nil {
// 						return err
// 					}
...
// 			)
// 			if err != nil {
// 				logs.Error(fmt.Sprintf("任务启动失败 %s(%d-%s %s): %s", t.Title, t.ID, clientTitle, proxyTitle, err.Error()))
// 				return
// 			}
// 			ttr.RunNow()
// 			rt.runners[ttr.RunID] = ttr
...
// 	// runid := fmt.Sprintf("%d", t.ID)
// 	// if tt := RunnerTask.Query(runid); tt != nil{
// 	// 	RunnerTask.Stop()
// 	// }
// 	if rta, ok := WatchingTasks.Load(t.ID); ok {
// 		if rt, ok := rta.(*RuningTask); ok {
// 			rt.Stop()
// 			// RunnerTask.Stop(fmt.Sprintf("%d", t.ID))
// 		}
// 	}
// 	return t.Start()
// }
