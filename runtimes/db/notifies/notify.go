...
func init() {
	db.DB.AutoMigrate(&Notify{})
}

func (this *Notify) Save(tx *gorm.DB) error {
	if tx == nil {
		tx = db.DB
	}
	var err error
	if this.Id > 0 {
		err = tx.Model(&Notify{}).Where("id = ?", this.Id).
...
// 获取通知总数
func GetNotifysTotal(adminid int64) int64 {
	var total int64
	db.DB.Model(&Notify{}).
...
// 分页获取通知消息
func GetNotifys(page, limit int, adminid int64) []*Notify {
	if page < 1 {
		page = 1
	}
	if limit < 1 {
