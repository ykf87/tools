...
type Config struct {
	...
	Key        string `json:"key" gorm:"uniqueIndex;not null;type:varchar(32)" update:"false"`
	Value      string `json:"value" gorm:"default:null;"`
	Remark     string `json:"remark" gorm:"default:null"`
	Addtime    int64  `json:"addtime" gorm:"index;default:0" update:"false"`
...
func init() {
	db.DB.DB().AutoMigrate(&Config{})
	var lst []*Config
	db.DB.DB().Model(&Config{}).Find(&lst)
	for _, c := range lst {
...
// 			return txx.Model(&Config{}).Where("id = ?", this.Id).
...
func GetList(page, limit int) []*Config {
	...
	db.DB.DB().Model(&Config{}).Order("id desc").Offset((page - 1) * limit).Limit(limit).Find(&lst)
	return lst
}
