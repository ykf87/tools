...
func AddNewSub(urlStr, name string) (*Subscribe, error){
	...
	db.DB.Model(&Subscribe{}).Where("url_md5 = ?", urlmd5).First(subrow)
...
func (this *Subscribe) GetFromServer() ([]*Proxy, error){
	...
	db.DB.Where("subscribe = ?", this.Id).Delete(&Proxy{})
	...
	if len(newProxys) > 0{
		db.DB.Model(&Proxy{}).Create(newProxys)
	}
	this.Total = len(resp)
	this.Save(nil)
...
func (this *Subscribe) Save(tx *gorm.DB) error {
	if tx == nil {
		tx = db.DB
	}
	if this.Id > 0 {
		return tx.Model(&Subscribe{}).Where("id = ?", this.Id).
