...
func (m meiPai) parseShareUrl(shareUrl string, transport *http.Transport) (*VideoParseInfo, error) {
	client := resty.New()
	if transport != nil {
...
func (m meiPai) parseVideoID(videoId string, transport *http.Transport) (*VideoParseInfo, error) {
	reqUrl := "https://www.meipai.com/video/" + videoId
	return m.parseShareUrl(reqUrl, transport)
}
