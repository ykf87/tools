...
func (a acFun) parseShareUrl(shareUrl string, transport *http.Transport) (*VideoParseInfo, error) {
	client := resty.New()
	if transport != nil {
	...
	parseInfo.Platform = "acfun"

	return parseInfo, nil
}

func (a acFun) parseVideoID(videoId string, transport *http.Transport) (*VideoParseInfo, error) {
	// acid, 格式: ac36935385
	reqUrl := "https://www.acfun.cn/v/" + videoId
	return a.parseShareUrl(reqUrl, transport)
}
