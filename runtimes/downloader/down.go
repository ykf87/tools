...
// 外部调用接口
func Download(ctx context.Context, opt *DownloadOption) (string, error) {
	opt.MainWait.Add(1)
	defer opt.MainWait.Done()
	return df(ctx, opt)
}

func df(ctx context.Context, opt *DownloadOption) (string, error) {
	...
	for {
		...
		case <-ticker.C:
			if atomic.LoadInt64(&progress) >= size {
				...
				writerWG.Wait()
				complated = true
				return name, os.Rename(partPath, filePath)
			}
		}
	}
}
